...
from app.state import running_tasks, create_task, stop_task, is_task_stopped, cleanup_task, get_task_info
...
async def main(prompt, task_id=None):
    ...
    while retry_count < max_retries:
        ...
        if task_id and is_task_stopped(task_id):
            logger.info(f"Task {task_id} stopped before execution")
            return
        ...
        if not result:
            ...
            max_wait_time = 10  # 10 seconds wait time
            wait_time = 5
        ...
        try:
            ...
            agent = await Manus.create(
                ...
                api_key=api_key,
                task_id=task_id  # Pass task_id to agent for stop checking
            )
            
            # Execute the task with stop checking
            await agent.run(prompt, task_id=task_id)
            ...
            elif "task stopped" in error_str:
                logger.info(f"Task {task_id} was stopped by user")
                break  # Don't retry if task was deliberately stopped
            else:
